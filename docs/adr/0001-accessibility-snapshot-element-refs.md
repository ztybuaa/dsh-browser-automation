...
快照选择器覆盖的原生标签（`a[href]`/`button`/`input`/`select`/`textarea`/`contenteditable`）与交互式 ARIA widget role（`listbox`/`option`/`tab`/`menuitem`/`menuitemcheckbox`/`menuitemradio`/`checkbox`/`radio`/`switch`/`searchbox`/`slider`/`spinbutton`/`treeitem`），以代码常量 `INTERACTIVE_ROLES` 为唯一事实源（Iteration 2 起，覆盖 Angular Material / MUI / shadcn 等自定义组件）。有状态元素（`aria-checked`/`aria-selected`/`aria-expanded`/`disabled`）在快照中带可选 `state` 字段，有值才显示。元素名解析按 `aria-label` → `aria-labelledby` → 关联 `<label for>` → `placeholder` → `value` → 文本 顺序（Iteration 3 起），且元素元数据经一次 `evaluateAll` 页面内往返批量收集、按 `maxElements`（默认 200）截断并标记 `truncated`（Iteration 3 起）。导航会清空旧 refs，防止模型引用上一页的编号（Iteration 3 起）。
